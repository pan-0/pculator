...
void rabbit_recalcmapping(rabbit_t* dev) {
    uint32_t shread;
    uint32_t shwrite;
    uint32_t shflags = 0;
...
void rabbit_write(void* priv, uint32_t addr, uint8_t val) {
...
uint8_t rabbit_read(void* priv, uint32_t addr) {
    uint8_t   ret = 0xff;
    rabbit_t* dev = (rabbit_t*)priv;
...
void rabbit_init() {
    ...
    ports_cbRegister(0x22, 2, rabbit_read, NULL, rabbit_write, NULL, &rabbit);
}
