...
#define MEM_READ_EXTANY 1
#define MEM_WRITE_INTERNAL 2
#define MEM_READ_INTERNAL 4
#define MEM_WRITE_DISABLED 8
#define MEM_WRITE_EXTANY 16

static void
opti495_recalc(opti495_t *dev)
{    uint32_t base, shadowbios, shadowbios_write;
    uint32_t shflags = 0;
    ...
    //printf("Shadow BIOS = %s, Base %08X\n", shadowbios ? "Yes" : "No", base);
...
void opti495_write(void* priv, uint32_t addr, uint8_t val)
{
    ...
    switch (addr) {
        ...
        case 0x22:
            opti495_log("[W] dev->idx = %02X\n", val);
            dev->idx = val;
            break;
        case 0x23:
        case 0x24:
            if ((dev->idx >= 0x20) && (dev->idx <= dev->max)) {
                opti495_log("[W] dev->regs[%04X] = %02X\n", dev->idx, val);
...
uint8_t opti495_read(void* priv, uint32_t addr)
{
    ...
    switch (addr) {
        case 0x22:
            opti495_log("[R] dev->idx = %02X\n", ret);
            break;
        case 0x23:
        case 0x24:
            if ((dev->idx >= 0x20) && (dev->idx <= dev->max)) {
                ret = dev->regs[dev->idx];
                opti495_log("[R] dev->regs[%04X] = %02X\n", dev->idx, ret);
            }
...
void opti495_init()
{
    opti495_t* dev = &opti495; //(opti495_t *) calloc(1, sizeof(opti495_t));

    //device_add(&port_92_device);

    ports_cbRegister(0x0022, 0x0003, opti495_read, NULL, opti495_write, NULL, dev);
    ports_cbRegister(0x00E1, 0x0002, opti495_read, NULL, opti495_write, NULL, dev);
