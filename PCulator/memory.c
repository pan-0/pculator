...
uint8_t mem_map_lookup[1 << 20]; //4 KB page lookup for emulator memory ranges
...
uint8_t cpu_read_linear(CPU_t* cpu, uint32_t addr32) {
	int map;
	uint8_t cacheval;

	if (!cpu->a20_gate) addr32 &= 0xFFFFF;
...
FUNC_FORCE_INLINE uint32_t translate_page(CPU_t* cpu, uint32_t addr32, int iswrite) {
	...
	if (map == 0xFF) {
		...
		tentry = 0;
		cpu->cr[2] = addr32;
		exception(cpu, 14, (iswrite ? 2 : 0) | ((cpu->startcpl == 3) ? 4 : 0)); //PF
		return 0xFFFFFFFF;
	}
	...
	if ((((tentry & 2) == 0) || ((dentry & 2) == 0)) && iswrite) { //If page not writable
		...
		if (!(cpu->cr[0] & 0x00010000) && (cpu->startcpl < 3)) { //as long as WP bit isn't set in CR0, supervisor code can still write
		...
		if (!canwrite) {
			...
			cpu->cr[2] = addr32;
			exception(cpu, 14, ((cpu->startcpl == 3) ? 4 : 0) | 3); //PF
			return 0xFFFFFFFF;
		}
	}
...
void cpu_write_linear(CPU_t* cpu, uint32_t addr32, uint8_t value) {
	int map;

	if (cpu->nowrite) return;

	if (!cpu->a20_gate) addr32 &= 0xFFFFF; //addr32 &= 0xFFEFFFFF;
...
FUNC_INLINE void cpu_write(CPU_t* cpu, uint32_t addr32, uint8_t value) {
	int map;

	if (cpu->nowrite) return;

	if (cpu->cr[0] & 0x80000000) {
		addr32 = translate_page(cpu, addr32, 1);
...
FUNC_INLINE uint8_t cpu_read(CPU_t* cpu, uint32_t addr32) {
	int map;
	uint8_t cacheval;

	if (cpu->cr[0] & 0x80000000) {
		addr32 = translate_page(cpu, addr32, 0);
...
FUNC_INLINE void cpu_writel(CPU_t* cpu, uint32_t addr32, uint32_t value) {
	cpu_write(cpu, addr32, (uint8_t)value);
	cpu_write(cpu, addr32 + 1, (uint8_t)(value >> 8));
	cpu_write(cpu, addr32 + 2, (uint8_t)(value >> 16));
	cpu_write(cpu, addr32 + 3, (uint8_t)(value >> 24));
}

void cpu_writew_linear(CPU_t* cpu, uint32_t addr32, uint16_t value) {
	cpu_write_linear(cpu, addr32, (uint8_t)value);
	cpu_write_linear(cpu, addr32 + 1, (uint8_t)(value >> 8));
}

void cpu_writel_linear(CPU_t* cpu, uint32_t addr32, uint32_t value) {
	cpu_write_linear(cpu, addr32, (uint8_t)value);
	cpu_write_linear(cpu, addr32 + 1, (uint8_t)(value >> 8));
	cpu_write_linear(cpu, addr32 + 2, (uint8_t)(value >> 16));
	cpu_write_linear(cpu, addr32 + 3, (uint8_t)(value >> 24));
}
...
uint16_t cpu_readw_linear(CPU_t* cpu, uint32_t addr32) {
	return ((uint16_t)cpu_read_linear(cpu, addr32) | (uint16_t)(cpu_read_linear(cpu, addr32 + 1) << 8));
}
