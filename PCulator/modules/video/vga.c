...
volatile uint16_t vga_curScanline = 0, vga_chain4, vga_modeY;
	...
	case VGA_MODE_TEXT:
		...
		for (scy = start_y; scy <= end_y; scy++) {
			...
			for (scx = start_x; scx <= end_x; scx++) {
				...
				if (attr & 0x80) {
					fontdata = vga_cursor_blink_state ? fontdata : 0;
				}
				attr &= 0x7F;
	...
	case VGA_MODE_GRAPHICS_8BPP:
		for (scy = start_y; scy <= end_y; scy += yscanpixels) {
			...
			for (scx = start_x; scx <= end_x; scx += xscanpixels) {
				...
				//if (vga_chain4 || (vga_gfxd[5] == 0x40)) { //Chain4
				if (vga_chain4 || (vga_modeY && !vga_chain4)) {
					addr = ((y * xstride) + x);
					plane = addr & 3;
					...
					addr = ((y * (uint32_t)vga_crtcd[0x13] * 2) + (x >> 2)) & 0xFFFF;
					plane = x & 3;
					addr += startaddr;
					cc = vga_RAM[plane][addr & 0xFFFF];
				}
...
void vga_writeport(void* dummy, uint16_t port, uint8_t value) {
	...
	case 0x3C5:
		if (vga_seqi < 0x05) {
			...
			case 0x02:
				...
				break;
			case 0x04:
				vga_chain4 = (vga_seqd[4] & 8) ? 1 : 0;
				//printf("Chain4: %s\n", vga_chain4 ? "enabled" : "disabled");
				break;
			}
		}
		break;
	...
	if (((vga_seqd[4] & 0xC) == 4) && ((vga_gfxd[5] & 0xB) == 0) && ((vga_gfxd[6] & 0x2) == 0) && ((vga_crtcd[20] & 0x40) == 0) && (vga_crtcd[23] & 0x40)) {
		vga_modeY = 1;
	}
	else {
		vga_modeY = 0;
...
void vga_writememory(void* dummy, uint32_t addr, uint8_t value) {
	...
	if (vga_seqd[0x04] & 0x08) { //chain-4
		...
		//vga_RAM[0][addr] = value;
		//vga_RAM[1][addr] = value;
		//vga_RAM[2][addr] = value;
		//vga_RAM[3][addr] = value;
		return;
	}
