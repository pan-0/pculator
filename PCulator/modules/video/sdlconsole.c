...
uint8_t sdlconsole_scancodeSet, sdlconsole_curkey, sdlconsole_lastKey, sdlconsole_frameIdx = 0, sdlconsole_grabbed = 0, sdlconsole_ctrl = 0, sdlconsole_alt = 0, sdlconsole_doRepeat = 0;
...
int sdlconsole_init(char *title) {
	...
	sdlconsole_scancodeSet = 1; //If the machine that runs uses scancode set 2 (AT-style) then this will be changed later during machine init.
...
void sdlconsole_changeScancodes(uint8_t keyset) {
	sdlconsole_scancodeSet = keyset;
}
	...
	switch (event.type) {
		case SDL_KEYDOWN:
			...
			default:
				...
				else if (sdlconsole_scancodeSet == 2) {
					sdlconsole_curkey = sdlconsole_translateScancodeSet2(event.key.keysym.sym, 0);
					//sdlconsole_lastKey = sdlconsole_curkey;
					//timing_updateIntervalFreq(sdlconsole_keyTimer, 2);
					//timing_timerEnable(sdlconsole_keyTimer);
					if (kbc.config & 0x40) { //XT/set 1 translation mode
						uint8_t key = sdlconsole_translateScancode(event.key.keysym.sym);
		...
		case SDL_KEYUP:
			...
			if (sdlconsole_scancodeSet == 1) {
				...
				if (kbc.config & 0x40) { //XT/set 1 translation mode
					uint8_t key = sdlconsole_translateScancode(event.key.keysym.sym) | 0x80;
					i8042_buffer_key_data(&key, 1, 1);
				}
				else { //native set 2 mode
...
uint8_t sdlconsole_translateScancodeSet2(SDL_Keycode keyval, int isbreak) {
	uint8_t i;
	for (i = 0; i < 104; i++) {
		if (keyval == sdlconsole_SDLtoSet2Map[i]) {
			break;
		}
	}
	return i;
