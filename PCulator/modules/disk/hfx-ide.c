...
#define IDE_LOG(x, ...) printf(x, ##__VA_ARGS__)
#define IDE_FATAL(x, ...) \
...
static uint64_t ide_get_sector_offset(struct ide_controller* ctrl, int lba48)
{
    ...
    case 1: // LBA24
        ...
        break;
    case 2: case 3: { // LBA48 commands override any IDE setting you may have set.
        res = ctrl->sector_number & 0xFF;
...
static void ide_write(uint32_t port, uint32_t data)
{
    ...
    case 0x1F7: {
        ...
        case 0x18: case 0x19: case 0x1A: case 0x1B: case 0x1C: case 0x1D: case 0x1E: case 0x1F: // Calibrate Drive
...
void ide_init(struct pc_settings* pc)
{
    ...
    for (int i = 0; i < 4; i++) {
        struct drive_info* info = &drives[i];
        ...
        ctrl->dma_enabled = 0; // pc->pci_enabled;
