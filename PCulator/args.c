...
int args_parse(MACHINE_t* machine, int argc, char* argv[]) {
	...
	for (i = 1; i < argc; i++) {
		if (args_isMatch(argv[i], "-h")) {
...
#ifdef USE_PCEM_IDE
				hdd_load_ext(&ide_drives[0].hdd_file, argv[++i], 63, 16, 1013, 0);
				ide_drives[0].type = IDE_HDD;
				ide_drives[0].skip512 = 0;
				hdc[0].hpc = ide_drives[0].hdd_file.hpc;
				hdc[0].spt = ide_drives[0].hdd_file.spt;
				hdc[0].tracks = ide_drives[0].hdd_file.tracks;
#else
...
#ifdef USE_PCEM_IDE
				hdd_load_ext(&ide_drives[1].hdd_file, argv[++i], 63, 16, 1013, 0);
				ide_drives[1].type = IDE_HDD;
				ide_drives[1].skip512 = 0;
				hdc[1].hpc = ide_drives[1].hdd_file.hpc;
				hdc[1].spt = ide_drives[1].hdd_file.spt;
				hdc[1].tracks = ide_drives[1].hdd_file.tracks;
#else
			...
			else if (args_isMatch(argv[i + 1], "text")) videocard = VIDEO_CARD_TEXT;
			else {
		...
		else if (args_isMatch(argv[i], "-int13emu")) {
			machine->int13emu = 1;
		}
